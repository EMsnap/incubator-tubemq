...
public class OffsetTimeManager extends AbstractDaemonService{
    ...
    public OffsetTimeManager(final BrokerConfig brokerConfig, final TubeBroker broker) {
        super("[Offset Time Manager]", 60000L);
    ...
    private void getAndSendMessages(Map<String, Map<String, Map<Integer, GroupOffsetInfo>>> groupOffsetMaps) {
        ...
        String time = f.format(messageTime);
        ...
        for (Map.Entry<String, Map<String, Map<Integer, GroupOffsetInfo>>> groupOffsetEntry
            ...
            pendingMessages.add(offsetMsgBuilder.toString());
            offsetMsgBuilder.delete(0, offsetMsgBuilder.length());
            sendMessages(time, pendingMessages);
        }
    }
    ...
    private void sendMessages(String time, List<String> pendingMessages) {
        ...
        for (String message : pendingMessages) {
            ...
            // attributes are
            String attributes = TokenConstants.TOKEN_MSG_TYPE + TokenConstants.EQ + time;
            byte[] bytes = encodePayload(body, attributes);
            // use time as filter item
            int msgTypeCode = time.hashCode();
            ...
            logger.info("send message {} at time {}", new String(bytes), time);
            try {
                ...
                if (!sendRes) {
                    logger.error("send offset time message {}, fail!", message);
                }
            } catch (final Throwable ex) {
                logger.error("send offset time message {}, fail with ex", message);
            }
        }
    }
